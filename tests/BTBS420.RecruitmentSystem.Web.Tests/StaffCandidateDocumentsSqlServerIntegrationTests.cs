...
public sealed class StaffCandidateDocumentsSqlServerIntegrationTests :
    IClassFixture<TestWebApplicationFactory>
{
    private const string ConnectionStringEnvironmentVariable =
        "KAN94_TEST_SQLSERVER_CONNECTION_STRING";
    ...
    public StaffCandidateDocumentsSqlServerIntegrationTests(TestWebApplicationFactory factory)
    {
        _baseFactory = factory;
    }

    [SqlServerIntegrationFact]
    public async Task Index_KapsamDahilindekiUzmanErisebilir()
    {
        ...
        var (profileId, _) = await CreateAppliedCandidateAsync(factory, runId, jobPostingId);

        using var specialistClient = CreateClient(factory);
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"/StaffCandidateDocuments/Index?candidateProfileId={profileId}");
    ...
    [SqlServerIntegrationFact]
    public async Task Index_KapsamDisindakiUzmanErisemezNotFoundDoner()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
    ...
    [SqlServerIntegrationFact]
    public async Task Index_DogruDepartmandakiYoneticiErisebilir()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
    ...
    [SqlServerIntegrationFact]
    public async Task Index_YanlisDepartmandakiYoneticiErisemezNotFoundDoner()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
        ...
        var otherDepartmentId = await CreateDepartmentAsync(setupClient, $"Kan94-OtherDept-{runId}");
        var managerId = await CreateHiringManagerUserAsync(
            factory,
            $"kan94-wrongmanager-{runId}",
            otherDepartmentId);
    ...
    [SqlServerIntegrationFact]
    public async Task Index_AdminHerZamanErisebilir()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
    ...
    [SqlServerIntegrationFact]
    public async Task Download_KapsamDahilindekiPersonelIndirebilirVeAuditKaydeder()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
        var (jobPostingId, _, responsibleUserId) = await CreatePublishedJobPostingAsync(setupClient, factory, runId);
        var (_, documentId) = await CreateAppliedCandidateAsync(factory, runId, jobPostingId);

        using var specialistClient = CreateClient(factory);
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"/StaffCandidateDocuments/Download?id={documentId}");
        request.Headers.Add(TestAuthenticationHandler.RoleHeaderName, SystemRoles.RecruitmentSpecialist);
        request.Headers.Add(TestAuthenticationHandler.UserIdHeaderName, responsibleUserId);
    ...
    [SqlServerIntegrationFact]
    public async Task Download_KapsamDisindakiPersonelIndiremezNotFoundDoner()
    {
        using var factory = CreateSqlFactory();
        var runId = Guid.NewGuid().ToString("N");
        using var setupClient = CreateClient(factory);
    ...
    private static async Task<(int ProfileId, int DocumentId)> CreateAppliedCandidateAsync(
        WebApplicationFactory<Program> factory,
        string runId,
        int jobPostingId)
    {
        var candidateId = $"kan94-candidate-{runId}";
    ...
    private static async Task<(int JobPostingId, int DepartmentId, string ResponsibleUserId)>
        CreatePublishedJobPostingAsync(
            ...
            string runId)
    {
        var departmentId = await CreateDepartmentAsync(client, $"Kan94-Dept-{runId}");

        var positionName = $"Kan94-Pos-{runId}";
        ...
        using (var request = new HttpRequestMessage(HttpMethod.Post, "/JobPostings/Create"))
        {
            ...
            request.Content = new FormUrlEncodedContent(
                new Dictionary<string, string>
                {
                    ...
                    ["Description"] = "Kan-94 entegrasyon testi ilanı.",
        ...
        public SqlServerIntegrationFactAttribute()
        {
                        ...
                        ConnectionStringEnvironmentVariable)))
            {
                Skip =
                    ...
                    "geçici SQL Server aday belgesi kapsam testi atlandı.";
            }
        }
    }
}
